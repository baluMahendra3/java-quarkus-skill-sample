...
- Treat the markdown files under `.github/copilot-skills/` as the primary repo guidance.
...
- If the tests touch framework behavior, request handling, CDI, profiles, or runtime wiring, also read the matching framework skill before editing.
...
- If the task involves build commands, test execution strategy, plugin behavior, or dependency changes, also read the matching build skill before editing.
...
- For Java code structure, naming, DTO or entity patterns, and maintainability, read `.github/copilot-skills/java-skill.md` first.
...
- For API contracts and OpenAPI or Swagger changes, read `.github/copilot-skills/swagger-skill.md`.
...
- For authentication, authorization, secrets, and secure defaults, read `.github/copilot-skills/security-skill.md`.
...
- The compliance script uses standard defaults and auto-detects module structure where possible.
- If this repository differs from those defaults, pass repo-specific values as script parameters or through the VS Code task before editing the script.